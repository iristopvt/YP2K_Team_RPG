...
❗ 오류 상황: 옵션창 UI 클릭 불가 현상 발생  
- 게임 플레이 중 장비창(스탯/인벤토리 창)이 열린 상태에서 옵션창을 함께 열 경우,  
  UI 우선순위(ZOrder) 충돌로 인해 옵션창 버튼 클릭이 되지 않음  
- 특히 옵션창을 띄우며 게임을 일시정지(Pause) 상태로 전환했기 때문에,  
  마우스 입력이 UI에 제대로 전달되지 않아 게임이 정지된 채로 조작 불가한 상태에 빠짐  

✅ 해결 방안: 인게임 옵션창 비활성화  
- 해당 구조상 UI 우선순위 정리 및 입력 포커스 재설정이 복잡하다고 판단  
- 게임 중에는 옵션창을 열 수 없도록 설계 변경  
  → 옵션창은 메인 메뉴 또는 타이틀 화면에서만 접근 가능하게 설정  
