...
🎮 플레이어 및 캐릭터 시스템 구현  
- AMyPlayer 및 ADragon 클래스 기반 조작 캐릭터 구현  
- StatComponent 연동으로 HP, MP, 스탯 및 보너스 포인트 성장 시스템 설계  
- 레벨업 시 능력치를 강화하고 UI에 실시간 반영  
  
🧠 몬스터 AI 및 보스 패턴 구현  
- 일반 몬스터와 차별화된 Epic Monster AI 구성  
- Behavior Tree + Blackboard를 통해 원거리 마법 / 근접 공격 / 소환 패턴을 혼합 적용  
- 특정 거리 이상일 땐 마법 구체 발사, 중거리엔 독 안개 생성, 근접 시 물리 공격  

🗺️ UI 및 미니맵 시스템 개발  
- HP/MP/EXP Bar, 스탯 창, 인벤토리, 상점, 스킬창 UI 구현  
- 미니맵 구현: 플레이어 월드 위치를 기반으로 회전 가능한 머테리얼 기반 미니맵 시스템 구축  
- 미니맵 상에 NPC, 몬스터, 아이템, 포탈 등의 아이콘을 분류 및 표시  
